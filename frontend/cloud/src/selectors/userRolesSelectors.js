...
const getOrganizationUserRoles = (state, props) =>
    state.userRoles.domainUserRoles
        ? state.userRoles.domainUserRoles["organization"] && state.userRoles.domainUserRoles["organization"][props.organizationID]
            ? state.userRoles.domainUserRoles["organization"][props.organizationID]
            : {}
        : {}

const getClinicUserRoles = (state, props) =>
    state.userRoles.domainUserRoles
        ? state.userRoles.domainUserRoles["clinic"] && state.userRoles.domainUserRoles["clinic"][props.clinicID]
            ? state.userRoles.domainUserRoles["clinic"][props.clinicID]
            : {}
        : {}
...
export const makeGetOrganizationUserUserRoles = () => {
    return createSelector([getUserUserRoles, getOrganizationID], (userRoles, organizationID) => {
...
export const makeGetClinicUserUserRoles = () => {
    return createSelector([getUserUserRoles, getClinicID], (userRoles, clinicID) => {
...
export const makeGetWildcardUserUserRoles = () => {
    return createSelector([getUserUserRoles], userRoles => {
...
export const makeGetUserAllowedClinicIDs = () => {
    return createSelector([makeGetUserOrganizationIDs(), getOrganizations], (userOrganizationIDs, organizations) => {
        let clinicIDs = []
        _.forEach(userOrganizationIDs, organizationID => {
            let organizationClinicIDs = organizations[organizationID]
                ? organizations[organizationID].clinics
                    ? organizations[organizationID].clinics
                    : []
                : []
...
export const makeGetOrganizationUserIDs = () => {
    return createSelector([getOrganizationUserRoles], userRoles => {
        return _.uniq(_.map(userRoles, "userID"))
    })
}

export const makeGetClinicUserIDs = () => {
    return createSelector([getClinicUserRoles], userRoles => {
        return _.uniq(_.map(userRoles, "userID"))
    })
}

export const makeGetUserOrganizationUserRoles = () => {
    return createSelector([getOrganizationUserRoles, getUserID], (userRoles, userID) => {
        return _.pickBy(userRoles, userRole => userRole.userID === userID)
    })
}

export const makeGetUserClinicUserRoles = () => {
    return createSelector([getClinicUserRoles, getUserID], (userRoles, userID) => {
        return _.pickBy(userRoles, userRole => userRole.userID === userID)
    })
}
